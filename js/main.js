(async function() {
  ...
  // Logo
  document.getElementById('logoPlace').innerHTML = D.site.logo || '<svg width="30" height="30" viewBox="0 0 100 100" fill="none"><rect width="100" height="100" rx="20" fill="#1E3D2F"/><path d="M50 20C38 20 28 30 28 42s10 22 22 22 22-10 22-22-10-22-22-22z" fill="#C8A25A" opacity=".85"/><path d="M36 52c0 0 6 24 14 32s14-32 14-32" stroke="#C8A25A" stroke-width="4" fill="none" stroke-linecap="round"/></svg><span class="logo-text">' + D.site.nome + '</span>';
  ...
  let current = 0, total = D.slides.length, interval;
  function goTo(idx) {
    if (idx<0) idx=total-1; if (idx>=total) idx=0; current=idx;
    track.style.transform = 'translateX(-' + (current*100) + '%)';
    dots.querySelectorAll('.dot').forEach((d,i) => d.classList.toggle('active', i===current));
  }
  dots.querySelectorAll('.dot').forEach(d => d.addEventListener('click', () => { clearInterval(interval); goTo(parseInt(d.dataset.index)); interval = setInterval(() => goTo(current+1), 5000); }));
  document.getElementById('carPrev')?.addEventListener('click', () => { clearInterval(interval); goTo(current-1); interval = setInterval(() => goTo(current+1), 5000); });
  document.getElementById('carNext')?.addEventListener('click', () => { clearInterval(interval); goTo(current+1); interval = setInterval(() => goTo(current+1), 5000); });
  ...
  // Conteúdo
  document.getElementById('conteudoGrid').innerHTML = D.conteudos.map((c,i) => '<div class="card-conteudo" data-idx="' + i + '" style="border-radius:12px;overflow:hidden;cursor:pointer"><img src="' + c.img + '" alt="" style="width:100%;aspect-ratio:16/9;object-fit:cover;display:block;background:#f0f0f0" onerror="this.style.display=\'none\'"><div style="padding:12px 0"><span style="font-size:0.65rem;font-weight:600;color:#C8A25A;text-transform:uppercase;letter-spacing:1px">' + c.cat + '</span><h3 style="font-size:0.9rem;font-weight:600;margin:4px 0;color:#333">' + c.titulo + '</h3><p style="font-size:0.78rem;color:#666">' + c.data + ' · ' + c.min + ' min</p></div></div>').join('');
  ...
  ['Produtos','Serviços','Contato'].forEach((tit,i) => {
    const links = i===0 ? [{texto:'Farmácia',url:'#produtos'},{texto:'Alimentos',url:'#produtos'},{texto:'Orgânicos',url:'#produtos'}]
      : i===1 ? [{texto:'Motorista',url:'#servicos'},{texto:'Saúde',url:'#servicos'},{texto:'Concierge',url:'#servicos'}]
      : [{texto:D.site.telefone,url:'tel:'+D.site.telefone},{texto:D.site.email,url:'mailto:'+D.site.email}];
    fg.innerHTML += '<div><h4>' + tit + '</h4><ul>' + links.map(l => '<li><a href="' + l.url + '">' + l.texto + '</a></li>').join('') + '</ul></div>';
  });
  ...
  function upd() {
    ...
    if (!cart.length) { el.innerHTML = '<p class="cart-empty">Vazio</p>'; return; }
  ...
  window.ayraCart = {
    add: (id,name,price,img) => { const e = cart.find(i => i.id===id); if (e) e.qty++; else cart.push({id,name,price,img,qty:1}); upd(); toast('Adicionado!'); },
    remove: i => { cart.splice(i,1); upd(); },
    qty: (i,d) => { cart[i].qty += d; if (cart[i].qty <= 0) cart.splice(i,1); upd(); }
  };
  document.addEventListener('click', e => {
    const btn = e.target.closest('.add-cart');
    if (btn) { e.stopPropagation(); const c = btn.closest('[data-id]'); if (c) window.ayraCart.add(c.dataset.id, c.dataset.name, parseFloat(c.dataset.price), c.dataset.img||''); }
  });
  ...
  document.getElementById('cartCheckout')?.addEventListener('click', () => {
    ...
    window.open('https://wa.me/5521999230233?text=Olá! Quero finalizar:\n' + encodeURIComponent(msg) + '\n\nTotal: R$ ' + total.toFixed(2).replace('.',','), '_blank');
  });
  upd();
  ...
  document.querySelectorAll('.card-conteudo').forEach(el => {
    el.addEventListener('click', () => {
      const c = D.conteudos[parseInt(el.dataset.idx)];
      if (!c) return;
      abrir('<span style="font-size:0.65rem;font-weight:600;color:#C8A25A;text-transform:uppercase;letter-spacing:1px">' + c.cat + '</span><h2 style="font-size:1.3rem;color:#1E3D2F;margin:8px 0">' + c.titulo + '</h2><img src="' + c.img + '" alt="" style="width:100%;border-radius:8px;margin-bottom:14px" onerror="this.style.display=\'none\'">' + c.html);
    });
  });

  document.querySelectorAll('#produtosGrid .card').forEach(el => {
    el.addEventListener('click', function() {
      const p = D.produtos.find(x => x.id === parseInt(this.dataset.id));
      if (!p) return;
      abrir('<img src="' + p.img + '" alt="" style="width:100%;border-radius:8px;margin-bottom:14px" onerror="this.style.display=\'none\'"><span style="font-size:0.65rem;font-weight:600;color:#C8A25A;text-transform:uppercase;letter-spacing:1px">' + p.cat + '</span><h2 style="font-size:1.3rem;color:#1E3D2F;margin:8px 0">' + p.nome + '</h2><p style="font-size:0.85rem;line-height:1.7;color:#666;margin-bottom:12px">' + p.desc + '</p><div style="font-size:1.2rem;font-weight:700;color:#C8A25A;margin-bottom:14px">R$ ' + p.preco.toFixed(2).replace('.',',') + '</div><button class="btn-primary add-cart" data-id="' + p.id + '" data-name="' + p.nome + '" data-price="' + p.preco + '" data-img="' + p.img + '" style="width:100%;justify-content:center">Adicionar ao carrinho</button>');
    });
  });

  // Newsletter
  document.getElementById('newsForm')?.addEventListener('submit', e => { e.preventDefault(); toast('Inscrição realizada!'); e.target.querySelector('input').value = ''; });
