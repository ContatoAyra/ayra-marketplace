(async function() {
  ...
  document.getElementById('conteudoGrid').innerHTML = D.conteudos.map((c, i) => '<div class="card-ct" data-idx="' + i + '" style="border-radius:12px;overflow:hidden;cursor:pointer"><img src="' + c.img + '" alt="" style="width:100%;aspect-ratio:16/9;object-fit:cover;background:#f0f0f0" onerror="this.style.display=\'none\'"><div style="padding:12px 0"><span style="font-size:0.65rem;font-weight:600;color:#C8A25A;text-transform:uppercase;letter-spacing:1px">' + c.cat + '</span><h3 style="font-size:0.9rem;font-weight:600;margin:4px 0;color:#333">' + c.titulo + '</h3><p style="font-size:0.78rem;color:#666">' + c.data + ' · ' + c.min + ' min</p></div></div>').join('');
  ...
  document.getElementById('footerBottom').textContent = '© 2026 ' + D.site.nome + '. Todos os direitos reservados.';
  ...
  window.ayraCart = { add: (id, n, p, img) => { const e = cart.find(i => i.id === id); if (e) e.qty++; else cart.push({ id, name: n, price: p, img, qty: 1 }); upd(); toast('Adicionado!'); }, remove: i => { cart.splice(i, 1); upd(); }, qty: (i, d) => { cart[i].qty += d; if (cart[i].qty <= 0) cart.splice(i, 1); upd(); } };
  ...
  document.getElementById('cartCheckout').addEventListener('click', () => { if (!cart.length) return; const msg = cart.map(i => i.name + ' x' + i.qty + ' = R$' + (i.price * i.qty).toFixed(2)).join('\n'); const t = cart.reduce((s, i) => s + i.price * i.qty, 0); window.open('https://wa.me/5521999230233?text=Olá! Quero finalizar:%0A' + encodeURIComponent(msg) + '%0A%0ATotal: R$ ' + t.toFixed(2).replace('.', ','), '_blank'); });
  upd();
